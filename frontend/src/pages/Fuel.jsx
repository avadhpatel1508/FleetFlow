...
const Fuel = () => {
    ...
    const createMutation = useMutation({
        mutationFn: async (newLog) => {
            const { data } = await api.post('/fuel', newLog);
            return data;
        },
        onSuccess: () => {
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <div>
                    <h1 className="text-2xl font-bold text-white tracking-tight">Fuel & Expenses Log</h1>
                    <p className="text-slate-400 mt-1">Track fleet fueling and associated costs</p>
                </div>
                <button
                        ...
                        <tbody>
                            {isLoading ? (
                                ...
                                <tr key={l._id} className="border-b border-slate-800 hover:bg-slate-800/20 transition-colors">
                                    ...
                                    <td className="px-6 py-4 font-mono text-white">{l.liters} L</td>
                                    <td className="px-6 py-4 text-rose-400 font-mono">${l.cost.toFixed(2)}</td>
                                    <td className="px-6 py-4">{new Date(l.date).toLocaleDateString()}</td>
                                </tr>
                            ))}
                            {!isLoading && logs.length === 0 && (
                                <tr><td colSpan="4" className="p-6 text-center">No fuel logs found.</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>
                    ...
                    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                        ...
                        >
                            <div className="flex justify-between items-center mb-6">
                                <h2 className="text-xl font-bold text-white">Log Fuel Expense</h2>
                            ...
                            <form onSubmit={handleSubmit} className="space-y-4">
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-slate-300 mb-1">Total Liters</label>
                                    <input required name="liters" type="number" step="0.1" min="0" className="w-full px-3 py-2 bg-slate-800/50 border border-slate-700 rounded-lg text-white outline-none focus:ring-2 focus:border-indigo-500" />
                                </div>
                                <div>
